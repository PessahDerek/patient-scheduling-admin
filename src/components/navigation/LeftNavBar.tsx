...
export default function LeftNavBar() {
  ...
  const paths: {
    ...
    .filter((f) => !["/auth", "/logout", "/about"].includes(f))
    .map((key) => {
      return {
        label: key == "/" ? "Overview" : capitalize(key.replace("/", "")),
        path: key,
      };
    })
  ...
  return (
    <nav
      className={
        "w-[200px] h-full bg-white flex flex-col gap-4 auto-rows-max items-center p-2"
      }
    >
      <ProfileBanner />
