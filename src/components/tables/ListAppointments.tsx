...
export default function ListAppointments({ list }: props) {
  ...
  const markComplete = (appointmentId: number) => {
    api
      .post("/admin/mark-app-complete", { appointmentId })
      .then((res) =>
        notifications.show({ title: "Sucess!", message: res.data.message }),
      )
        ...
        <Table.Tbody>
          {list.map((app) => (
            <Table.Tr key={app.id}>
              ...
              <Table.Td>
                <Badge variant={"light"} color={getColor(app.status)}>
                  {app.status}
                </Badge>
              </Table.Td>
              <Table.Td>
                <Menu onChange={(val) => setOpenMenu({ [app.id]: val })}>
                  ...
                  <Menu.Dropdown>
                    ...
                    <Menu.Item leftSection={<BiCalendarEdit />}>
                      Reschedule
                    </Menu.Item>
                    <Menu.Item
                      onClick={() =>
                        setCancel((p) => ({ ...p, appointment_id: app.id }))
                      }
                      c={"red"}
                      leftSection={<MdCancel />}
                    >
                      Cancel
                    </Menu.Item>
                  </Menu.Dropdown>
