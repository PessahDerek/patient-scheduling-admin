import { Badge, Button, Menu, Pill, Table, Text } from "@mantine/core";
...
export default function ListDaySchedule({ day, setToEdit }: props) {
  ...
  const getStatus = (s: ScheduleObj) => {
    const hrToday = new Date().getHours(),
      minToday = new Date().getMinutes();
    const timeOutHr =
      new Date(s.timeOut).getHours() == 0 ? 24 : new Date(s.timeOut).getHours();
    if (hrToday > timeOutHr || hrToday < new Date(s.timeIn).getHours())
  ...
  const getDocAppointment = useCallback(
    (schedule_id: number, doc_id: string) => {
      const found = [...appointments.values()].filter(
        (a) => a.schedule.id == schedule_id && dayjs(a.date).isSame(new Date()),
      );
      return found.length;
    },
    [appointments],
            ...
            <Table.Tr key={schedule.id}>
              ...
              <Table.Td>
                <Badge
                  title="flkajdfl"
                  variant="filled"
                  color={getStatus(schedule).c}
                >
                  {getStatus(schedule).t}
                </Badge>
              </Table.Td>
              <Table.Td>
                {getDocAppointment(schedule.id, schedule.doctor.id)}
              </Table.Td>
              <Table.Td>
                <Menu shadow={"md"}>
                  <Menu.Target>
